...
> R:24, agrega el precio del producto sin importar la cantidad

**P7** — ¿Qué debería hacer esa línea para calcular el total correctamente?

> R:Deberia multiplicar al producto por la cantidad 
...
> R:falla, devuleve 1 cuando deberia ser 0
...
> R:el incorrecto es <=, deberia ser < 
...
> R:si, las lineas 29 y 30, dentro de carrito_descuento. No se ejecutaron porque test_unitarios.c no llama a esa funcion, solo se prueba en test_integracion.c
...
> R:un test unitario prueba una funcion aislada, un test de integracion prueba que varias funciones trabajen bien combinadas. El test unitario "test_total_con_cantidad" detecto primero el bug.
...
> R:porque en Python y Java se realiza una verificacion de los tamaños del array/lista que no ocurre en C. En C escribir fuera de rango pisa memoria que no le corresponde generando un comportamniento no defiido.
...
> R:escribir tests permite que apartir de un resultado esperado se pueda comparar con lo que se obtine. Además los tests quedan guardados y se pueden volver a correr para asegurarse de que el bug no vuelva a aparecer.
...
> R:Porque el bug era no multiplicar por la cantidad, y con cantidad = 1 el valor esperado y el obtenido coinciden por casualidad. Sin emabrgo, cantidad = 2 detecta que solo se cuenta un item, ya que devulve 350 en vez de 700.
